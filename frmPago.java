   ...
   @SuppressWarnings("unchecked")
    ...
    private void btnguardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnguardarActionPerformed
        ...
        if (txtiva.getText().length() == 0) {
            JOptionPane.showConfirmDialog(rootPane, "Ingresar Comprobante de pago a generar");
            txtiva.requestFocus();
            return;
        }
        if (txttotal_pago.getText().length() == 0) {
            JOptionPane.showConfirmDialog(rootPane, "Ingresar el total de pago del comprobante");
            txttotal_pago.requestFocus();
            return;
        }

        if (txtnum_comprobante.getText().length() == 0) {
            JOptionPane.showConfirmDialog(rootPane, "Ingresar un Número de Comprobante del Pago");
        ...
        if (accion.equals("guardar")) {
            if (func.insertar(dts)) {
                JOptionPane.showMessageDialog(rootPane, " El pago por Q. " + txttotal_pago.getText() + 
